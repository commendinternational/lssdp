...
static int lssdp_send_response(lssdp_ctx * lssdp);
...
int lssdp_socket_read(lssdp_ctx * lssdp) {
    ...
    recv_len = recvfrom(lssdp->sock, buffer, sizeof(buffer), 0,
...
static int lssdp_send_response(lssdp_ctx * lssdp) {
    ...
    int response_len = snprintf(response, sizeof(response),
                                ...
                                lssdp->header.max_age,
                                lssdp->header.server,
                                lssdp->header.location.prefix,
                                lssdp->header.location.domain,
                                lssdp->header.location.suffix
                                );
    
    int ret = send_multicast_data(response, lssdp);
    if (ret < 0) {
        lssdp_error("failed to send multicast data\n");
        return -1;
    }
...
static int neighbor_list_add(lssdp_ctx * lssdp, const lssdp_packet packet) {
    ...
    lssdp_debug("neightboor list: %p packet st: %s usn: %s lssdp usn:%s\n", lssdp->neighbor_list, packet.st, packet.usn, lssdp->header.unique_service_name);
